...
async function checkAuthAndPlay() {
  ...
  if (!currentUser) {
    if (
      confirm("Sign in with Google to save scores? Cancel to play as guest.")
    ) {
  ...
  try {
    ...
    if (session?.user) {
      ...
      if (currentUser?.id !== session.user.id)
        console.log("User session active:", session.user.email); // Log only if user changes
      ...
      // User is logged out
      if (currentUser)
        console.log("No active user session found / User logged out."); // Log only if state changes
      currentUser = null;
      isGuest = false;
      currentGamingName = null;
    ...
    document
      ...
      ?.style.setProperty("display", "block", "important"); // Use ID
    updateUIName(null);
    return false;
  }
}
...
function showGamingNameModal(isUpdate = false) {
  ...
  if (isUpdate && !currentUser) {
    alert("Log in to change name");
    return;
  }
  if (isUpdate && currentGamingName && !canChangeGamingName()) {
    alert("Daily name change limit reached.");
    return;
  }
...
function closeGamingNameModal() {
  ...
  if (isGuest && !currentGamingName) {
    ...
    showScreen("home-screen");
  } // Logged-in user cancelled initial setup
}
...
async function checkGamingName() {
  ...
  try {
    const { data, error } = await supabase
      ...
      .maybeSingle();
    if (error) {
      console.error("Error fetching profile for gaming name:", error.message);
      currentGamingName = null;
      updateUIName("Error");
      return null;
    }
    ...
    console.error("Unexpected error fetching gaming name:", fetchError);
    currentGamingName = null;
    updateUIName("Error");
    return null;
  }
...
function updateUIName(name) {
  // Helper to update all UI elements needing the name
...
async function submitGamingName() {
  ...
  if (currentUser) {
    // Logged-in user flow
    if (isUpdate && gamingName !== currentGamingName && !canChangeGamingName())
    ...
    try {
      ...
      if (isUpdate && currentGamingName !== previousName)
        incrementNameChangeCounter(); // Alert is inside
      ...
      console.error("Error submitting gaming name:", error);
      alert(`Failed to save name: ${error.message}. Check RLS/Policies.`);
    }
  } else if (isGuest) {
    // Guest flow
    ...
    console.error("Submit called without user or guest state.");
  }
}
...
async function loadProfileStats() {
  if (!currentUser?.id) {
    console.log("Cannot load stats: No user.");
    /* Clear stats UI */ return;
  }
  ...
  try {
    ...
    const { data: allScores, error: rankError } = await supabase
      ...
      .order("score", { ascending: false }); // Only need score & profile_id for rank
    if (rankError) throw rankError;
    const userRankIndex = allScores.findIndex(
      (s) => s.profile_id === currentUser.id
    ); // Find first entry for user
...
function fetchLeaderboard() {
  ...
  document
    ...
    ?.addEventListener("click", () => showScreen("home-screen")); // Add listener for close button

  const content = document.getElementById(contentDivId);
  supabase
    .from("scores")
    .select("score,wpm,username,profiles:profile_id(gaming_name)")
    ...
    .then(({ data: scores, error }) => {
      ...
      content.innerHTML = "";
      content.style.textAlign = "left"; // Reset styles
      if (!scores?.length) {
        content.textContent = "No scores yet.";
        content.style.textAlign = "center";
        return;
      }
      scores.forEach((scoreData, index) => {
        let displayName =
          scoreData.profiles?.gaming_name || scoreData.username || "Player";
        const item = document.createElement("div");
        item.className = "leaderboard-item";
        item.innerHTML = `<div class="rank">#${
          index + 1
        }</div><div class="username">${displayName}</div><div class="score">${
          scoreData.score || 0
        }</div><div class="wpm">${scoreData.wpm || 0}</div>`;
        content.appendChild(item);
      });
    })
    .catch((fetchError) => {
      console.error("Fetch Leaderboard Promise Catch:", fetchError);
...
function updateScore() {
  ...
  const currentWpm =
    gameDurationMinutes > 0.05 && wordsTyped > 0
      ? Math.round(wordsTyped / gameDurationMinutes)
      : 0;
...
function gainLife() {
  if (lives < 3) {
    ...
    console.log("Max lives reached."); /* score += 10; updateScore(); */
  }
}
function loseLife() {
  if (lives > 0) {
...
} // Only trigger game over if game is running
...
function spawnObject() {
  ...
  const objectWidth = Math.max(100, word.length * 12);
  ...
  const baseMinSpeed = 1.5,
    ...
    speedFactor = 0.009,
    maxSpeed = 8.0;
  const increase = score * speedFactor;
  const minSpeed = Math.min(
    baseMinSpeed + increase,
    maxSpeed - (baseMaxSpeed - baseMinSpeed)
  );
  const maxSpeedNow = Math.min(baseMaxSpeed + increase, maxSpeed);
...
function checkAndSpawnObjects() {
  ...
  if (fallingObjects.length < maxObjects && Math.random() < spawnProb * 0.1) {
    spawnObject();
  }
}
...
function spawnPowerup() {
  ...
  activePowerups.push({ element: powerup, type: type, y: -60 });
  ...
  switch (type) {
    ...
    case "slowdown":
      ...
      fallingObjects.forEach((obj) => {
        if (!obj.originalSpeed) obj.originalSpeed = obj.speed;
        obj.speed = obj.originalSpeed * 0.5;
      });
      slowdownTimeout = setTimeout(() => {
        slowdownActive = false;
        fallingObjects.forEach((obj) => {
          if (obj.originalSpeed) obj.speed = obj.originalSpeed;
    ...
    case "destroy-all":
      ...
      score += points;
      wordsTyped += fallingObjects.length;
      updateScore();
...
function startGame() {
  if ((!isGuest && !currentGamingName) || (isGuest && !currentGamingName)) {
    console.error("StartGame called but name not set.");
    showGamingNameModal(false);
    return;
  }
  initGame();
  ...
  spawnObject();
  setTimeout(spawnObject, 1500 + Math.random() * 1000); // Spawn second after delay
...
function updateGame() {
  ...
  const h = gameArea.offsetHeight,
    boundary = h - 60; // Y-coord where words are missed
  ...
  for (let i = fallingObjects.length - 1; i >= 0; i--) {
    ...
    if (obj.y > boundary) {
      /* console.log(`Word '${obj.word}' missed`); */ removeObject(i);
      loseLife();
    }
  }
  // Update powerups
...
async function gameOver() {
  ...
  if (typingSpeedEl) {
    typingSpeedEl.textContent = finalWpm;
  }
...
async function saveScore(finalScore, finalWpm) {
  ...
  try {
    ...
    if (!currentUser?.id) {
      console.error("Save Error: Not logged in.");
      alert("Log in to save score.");
      return;
    }
    if (!currentGamingName) {
      console.error("Save Error: Gaming name not set.");
      alert("Set gaming name in profile to save score.");
      return;
    }

    console.log(
      `Saving score via Edge Function for ${currentUser.id}: ${finalScore} score, ${finalWpm} WPM`
    );
    ...
    if (error) {
      ...
      alert(`Save failed: ${error.message || "Unknown function error"}`);
    } else {
    ...
    alert("Unexpected error saving score.");
  } finally {
    fetchLeaderboard();
  } // Always refresh leaderboard after attempt
}
// =======================================================
...
document.addEventListener("DOMContentLoaded", () => {
  if (!supabase) {
    console.error("DOM loaded but Supabase client is not initialized.");
    return;
  }
  ...
  document
    ...
    ?.addEventListener("click", closeGamingNameModal); // Use ID
  document
    .getElementById("submit-name-btn")
    ?.addEventListener("click", submitGamingName); // Use ID
  document
    .getElementById("profile-close-btn")
    ?.addEventListener("click", () => showScreen("home-screen")); // Use ID
  document
    .getElementById("change-name-btn")
    ?.addEventListener("click", () => showGamingNameModal(true)); // Use ID
  document
  ...
  document
    ...
    ?.addEventListener("click", restartGame); // Use ID
  document
    .getElementById("end-leaderboard-btn")
    ?.addEventListener("click", () => showScreen("leaderboard-screen")); // Use ID
  document
  ...
  if (wordInputEl) {
    wordInputEl.addEventListener("input", (e) => {
      ...
      fallingObjects.forEach((obj, i) => {
        if (text === obj.word && obj.y > lowestMatchY) {
          lowestMatchY = obj.y;
          lowestMatchIndex = i;
        }
      });
      if (lowestMatchIndex !== -1) {
        const obj = fallingObjects[lowestMatchIndex];
        const points = obj.word.length;
        score += points;
        wordsTyped++;
        updateScore();
  ...
  checkAuth().then(() => {
    console.log("Initial auth check complete after DOM load.");
    if (!document.querySelector(".screen.active")) showScreen("home-screen");
  });
  supabase.auth.onAuthStateChange((event, session) => {
    console.log(
      "Auth state changed:",
    ...
    if (event === "SIGNED_IN") {
      checkGamingName();
    } // Ensure name check happens after sign in confirms user
  });
